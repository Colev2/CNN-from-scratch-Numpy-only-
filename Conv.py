...
class Conv_layer:
    ...
    def forward_old(self, input_img):
    ...
    def forward_new(self, input_img: np.ndarray) -> np.ndarray:
        input_img = np.asarray(input_img, np.float32)
        ...
        flat_windows = np.reshape(windows, shape=(batch_size * windows.shape[1] * windows.shape[2], Kh * Kw * img_channels))  # (B*Hout*Wout, Kh*Kw*C)
        flat_weights = np.reshape(self.weights, shape=(self.filters, Kh * Kw * img_channels))  # (F, Kh*Kw*C)
    ...
    def backward_new(self, dout: np.ndarray) -> np.ndarray:
        if self.padded_input is None:
            raise ValueError("Conv: padded_input attribute wasn't initialized. Forward method needs to be called")
