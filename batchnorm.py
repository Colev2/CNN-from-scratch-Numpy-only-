...
def main(): 
    ...
    dout = rng.normal(size=x.shape).astype(np.float64)
    ...
    for idx in np.ndindex(x.shape):
        ...
        dx_num[idx] = (loss_plus - loss_minus) / (2 * eps)


    # ---------- Numerical dgamma ----------
    dgamma_num = np.zeros_like(bn.gamma)

    for i in range(bn.gamma.size):
        old_value = bn.gamma[i]

        bn.gamma[i] = old_value + eps
        loss_plus = loss()

        bn.gamma[i] = old_value - eps
        loss_minus = loss()

        bn.gamma[i] = old_value

        dgamma_num[i] = (loss_plus - loss_minus) / (2 * eps)


    # ---------- Numerical dbeta ----------
    dbeta_num = np.zeros_like(bn.beta)

    for i in range(bn.beta.size):
        old_value = bn.beta[i]

        bn.beta[i] = old_value + eps
        loss_plus = loss()
        ...
        dbeta_num[i] = (loss_plus - loss_minus) / (2 * eps)


    print("dx max abs error:",
        np.max(np.abs(dx - dx_num)))

    print("dgamma max abs error:",
        np.max(np.abs(dgamma - dgamma_num)))

    print("dbeta max abs error:",
        np.max(np.abs(dbeta - dbeta_num)))
