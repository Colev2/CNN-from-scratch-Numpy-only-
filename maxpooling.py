...
class MaxPooling2D_layer:
    ...
    def forward(self, input: np.ndarray) -> np.ndarray:
        ...
        self.Hout = int(np.floor((height - Ph) / self.stride)) + 1
        self.Wout = int(np.floor((width - Pw) / self.stride)) + 1
        self.output_shape = (batch_size, self.Hout, self.Wout, self.in_channels)  # (B,Hout,Wout,Ch)
        ...
        reshaped_windows = np.reshape(pooling_windows, shape=(batch_size,self.Hout,self.Wout,self.in_channels,-1))    # (B,Hout,Wout,Ch,Ph*Pw)
    ...
    def backward(self, dout: np.ndarray) -> np.ndarray:
        ...
        for pr in range(self.pool_size[0]):
            for pc in range(self.pool_size[1]):
                ...
                din[:, pr:pr + self.Hout * self.stride:self.stride, pc:pc + self.Wout * self.stride:self.stride, :] += contribution
